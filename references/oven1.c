...
extern int  SetDough1(void) 
{
    ...
    if( status == 0 )
    {
        if( (GetPosConveyor() <= KLIN1_MAX) && (GetPosConveyor() >= KLIN1_MIN ) )
        {
                status = 1;     // Working
                if( state == 0 )
                {
                    state = 1;          // A dough is there in the oven1
...
extern int  StartBake1(int param) 
{
    ...
    if( status == 0 )
    {
        ...
        if( state == 1 )
        {
            state = 2;     // A dough is there in the oven1
...
extern int  WaitBake1(void) 
{
    ...
    if( status == 0 )
    {
        ...
        if( state == 2 )
        {
            state = 3;     // A dough is there in the oven1
